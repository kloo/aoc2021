...
Function Check-Num([int[]] $myNum,[String[]] $myStrarray) {
    $vars = @(0) * 4 #w,x,y,z

    foreach ($line in $myStrarray) {
        $command = $line.Split(" ")
        $varOne = -1
        $varTwoVal = -1
...
while ($true) {
    ...
    $outOne   = Start-Job -ScriptBlock ${function:Check-Num} -ArgumentList @($inputNum,$strarray)
    ...
    $outTwo   = Start-Job -ScriptBlock ${function:Check-Num} -ArgumentList @($inputNum,$strarray)
    ...
    $outThree = Start-Job -ScriptBlock ${function:Check-Num} -ArgumentList @($inputNum,$strarray)
    ...
    $outFour  = Start-Job -ScriptBlock ${function:Check-Num} -ArgumentList @($inputNum,$strarray)
